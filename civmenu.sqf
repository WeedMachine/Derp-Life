...
ctrlSetText [1, format["%1 gave you a ticket of $%2. Would you like to pay it?", _cop, (_strafe call ISSE_str_IntToStr)]];	
...
while {true} do 
	...
	if (player Distance prison_logic >= 100) exitWith {_exitart = "ausbruch"};
...
(format ['if (rolestring == "%1") then {[0, 0, 0, ["inventorysteal", %2, %3]] execVM "maindialogs.sqf";};hint format [localize "STRS_civmenucheck_stolenpub", %3];',_aktionsStarter, INV_InventarArray, player]) call broadcast;
