...
licenseflag2 sidechat "As soon as you go through the door to your right side the test will start. good luck!";
...
if(!(player hasweapon "M9"))exitwith
	...
	licenseflag2 sidechat "You have failed the test for dropping your weapon!";
	_score = 0;
...
if(_score >= 55)then
	...
	licenseflag2 sidechat format["Excellent work! %1 you successfully passed the test and have received your pistol license!", name player];
	INV_LizenzOwner = INV_LizenzOwner + ["semiautomatic"];
	INV_LizenzOwner = INV_LizenzOwner - ["gunprobationpistol"];
	["INV_LizenzOwner", INV_LizenzOwner] spawn ClientSaveVar;

	}else{licenseflag2 sidechat format["sorry %1 , but you have failed the test! Try again!", name player]};
