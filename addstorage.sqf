...
(_arrname call INV_StorageIsFactory) and 
	...
	if ([_item, -(_menge), _arrname2] call INV_AddItemStorage) then //zieht items von auto ab

		{

		[_item, _menge,_arrname] call INV_AddItemStorage; 		//fügtt fabrik hinzu
		player groupChat format[localize "STRS_inv_storage_took", (_menge call ISSE_str_IntToStr)];
		_packed2 = true;
...
if(_vclStore)then
	...
	_storageweight = ( (_arrname2 call INV_GetStorageWeight) + (_menge * (_infos call INV_getitemTypeKg)) );
	
	};

if(_storageweight > _maxweight)exitwith{player groupchat "The vehicle's storage is full";};

if (not([_item, -(_menge),_arrname] call INV_AddItemStorage)) then
	...
	if ([_item, _menge, _arrname2, _vclClass] call INV_AddItemStorage) then 
...
if (_packed2) then 
	...
	if (_public == "public") then {publicVariable _arrname2;};
